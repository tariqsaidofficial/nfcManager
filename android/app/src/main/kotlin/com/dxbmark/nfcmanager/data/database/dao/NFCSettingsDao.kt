package com.dxbmark.nfcmanager.data.database.dao
...
interface NFCSettingsDao {
    ...
    @Query("SELECT selectedLanguageCode FROM nfc_settings WHERE id = 1") // <<< NEW FUNCTION
    fun getSelectedLanguageCode(): Flow<String?> // <<< NEW FUNCTION
    ...
    @Query("UPDATE nfc_settings SET customNotificationSoundUri = :soundUri WHERE id = 1")
